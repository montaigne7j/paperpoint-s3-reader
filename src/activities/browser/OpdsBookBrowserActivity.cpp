...
void OpdsBookBrowserActivity::render(RenderLock&&) {
  ...
  const char* searchLabel =
      (!searchTemplate.empty() && selectorIndex == 0)
          ? tr(STR_SEARCH)
          : (ButtonNavigator::hasPreviousPage(selectorIndex, static_cast<int>(entries.size()), PAGE_ITEMS)
                 ? tr(STR_DIR_UP)
                 : "");
  const char* nextPageLabel =
      ButtonNavigator::hasNextPage(selectorIndex, static_cast<int>(entries.size()), PAGE_ITEMS) ? tr(STR_DIR_DOWN) : "";
...
void OpdsBookBrowserActivity::fetchFeed(const std::string& path) {
  ...
  std::string url = UrlUtils::buildUrl(SETTINGS.opdsServerUrl, path);
