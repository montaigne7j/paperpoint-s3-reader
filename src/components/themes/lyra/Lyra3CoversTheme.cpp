...
// Recent-book metadata may contain Chinese. SMALL_FONT_ID is the built-in
// Latin-only small font, so use the external UI font path instead.
constexpr int recentBookTextFontId = UI_10_FONT_ID;
}  // namespace
  ...
  if (hasContinueReading) {
    ...
    for (int i = 0; i < std::min(static_cast<int>(recentBooks.size()), Lyra3CoversMetrics::values.homeRecentBooksCount);
      ...
      for (const auto& line : titleLines) {
        renderer.drawText(recentBookTextFontId, tileX + hPaddingInSelection, currentY, line.c_str(), true);
