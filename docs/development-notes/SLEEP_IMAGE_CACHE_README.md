...
兩者都支援多層子資料夾。選擇「隨機」時會遞迴掃描上述兩個資料夾，並相容掃描舊版：
...
資料夾可逐層進入。圖片列第一次點選只移動游標，第二次點選或按下確認鍵才開啟圖片預覽。預覽底部有「取消 / 確認」：兩者都回到原資料夾，確認會記錄完整圖片路徑並要求背景快取重新準備；取消不修改設定。

自選模式的候選清單只包含指定檔案；切回隨機後才重新從所有目錄選圖。
