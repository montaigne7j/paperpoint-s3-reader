...
constexpr size_t TARGET_PIXELS = static_cast<size_t>(TARGET_WIDTH) * TARGET_HEIGHT;
...
bool shouldCancel(const TaskControl& control) { return control.cancel != nullptr && *control.cancel; }

bool pauseForForeground(const TaskControl& control) {
  while (!shouldCancel(control) && millis() - *control.lastActivityMs < PAUSE_AFTER_ACTIVITY_MS) {
...
uint32_t fnv1aString(const std::string& value) {
  return fnv1aUpdate(2166136261u, reinterpret_cast<const uint8_t*>(value.data()), value.size());
}
...
inline void setPackedNibble(uint8_t* buffer, int width, int x, int y, uint8_t value) {
  const size_t index = static_cast<size_t>(y) * static_cast<size_t>(width / 2) + static_cast<size_t>(x >> 1);
  value &= 0x0F;
  if ((x & 1) == 0) {
...
CropRect calculateCenterCrop(int sourceWidth, int sourceHeight) {
  ...
  const int64_t sourceCross = static_cast<int64_t>(sourceWidth) * TARGET_HEIGHT;
  const int64_t targetCross = static_cast<int64_t>(TARGET_WIDTH) * sourceHeight;

  if (sourceCross > targetCross) {
    crop.width = static_cast<int>((static_cast<int64_t>(sourceHeight) * TARGET_WIDTH) / TARGET_HEIGHT);
    crop.width = std::max(1, std::min(crop.width, sourceWidth));
    crop.x = (sourceWidth - crop.width) / 2;
  } else if (sourceCross < targetCross) {
    crop.height = static_cast<int>((static_cast<int64_t>(sourceWidth) * TARGET_HEIGHT) / TARGET_WIDTH);
...
void calculateFitRect(int sourceWidth, int sourceHeight, int& outX, int& outY, int& outWidth, int& outHeight) {
...
uint8_t bilinearSample(const uint8_t* source, int sourceWidth, int sourceHeight, uint32_t sourceX16,
                       uint32_t sourceY16) {
  ...
  const uint32_t top = (static_cast<uint32_t>(source[y0 * sourceWidth + x0]) * (65536u - fx) +
                        static_cast<uint32_t>(source[y0 * sourceWidth + x1]) * fx) >>
                       16;
  const uint32_t bottom = (static_cast<uint32_t>(source[y1 * sourceWidth + x0]) * (65536u - fx) +
                           static_cast<uint32_t>(source[y1 * sourceWidth + x1]) * fx) >>
                          16;
  return static_cast<uint8_t>((top * (65536u - fy) + bottom * fy) >> 16);
}

void bilinearSampleOverlay(const uint8_t* sourceGray, const uint8_t* sourceAlpha, int sourceWidth, int sourceHeight,
                           uint32_t sourceX16, uint32_t sourceY16, uint8_t& grayOut, uint8_t& alphaOut) {
  int x0 = static_cast<int>(sourceX16 >> 16);
  int y0 = static_cast<int>(sourceY16 >> 16);
  ...
  const auto lerp16 = [](uint32_t a, uint32_t b, uint32_t fraction) {
    return static_cast<uint32_t>(
        (static_cast<uint64_t>(a) * (65536u - fraction) + static_cast<uint64_t>(b) * fraction) >> 16);
  };
  const auto alphaAt = [&](int x, int y) -> uint32_t { return sourceAlpha[static_cast<size_t>(y) * sourceWidth + x]; };
  ...
  const uint32_t premulTop = lerp16(premultipliedAt(x0, y0), premultipliedAt(x1, y0), fx);
  const uint32_t premulBottom = lerp16(premultipliedAt(x0, y1), premultipliedAt(x1, y1), fx);
  const uint32_t premul = lerp16(premulTop, premulBottom, fy);

  alphaOut = static_cast<uint8_t>(std::min<uint32_t>(255, alpha));
  grayOut = alphaOut == 0 ? 255 : static_cast<uint8_t>(std::min<uint32_t>(255, (premul + alphaOut / 2u) / alphaOut));
}
...
bool resampleOpaque(const uint8_t* source, int sourceWidth, int sourceHeight, uint8_t* destination,
  ...
  for (int y = 0; y < TARGET_HEIGHT; ++y) {
    ...
    const uint32_t sourceY16 = static_cast<uint32_t>(crop.y << 16) +
                               static_cast<uint32_t>((static_cast<uint64_t>(y) * crop.height << 16) / TARGET_HEIGHT);
    for (int x = 0; x < TARGET_WIDTH; ++x) {
      const uint32_t sourceX16 = static_cast<uint32_t>(crop.x << 16) +
                                 static_cast<uint32_t>((static_cast<uint64_t>(x) * crop.width << 16) / TARGET_WIDTH);
...
bool resampleOverlay(const uint8_t* sourceGray, const uint8_t* sourceAlpha, int sourceWidth, int sourceHeight,
                     uint8_t* destination, const TaskControl& control) {
  ...
  for (int y = 0; y < outHeight; ++y) {
    if ((y & 31) == 0 && !pauseForForeground(control)) return false;
    const uint32_t sourceY16 = static_cast<uint32_t>((static_cast<uint64_t>(y) * sourceHeight << 16) / outHeight);
    for (int x = 0; x < outWidth; ++x) {
      const uint32_t sourceX16 = static_cast<uint32_t>((static_cast<uint64_t>(x) * sourceWidth << 16) / outWidth);
      uint8_t gray = 255;
      uint8_t alpha = 0;
      bilinearSampleOverlay(sourceGray, sourceAlpha, sourceWidth, sourceHeight, sourceX16, sourceY16, gray, alpha);
      destination[static_cast<size_t>(outY + y) * TARGET_WIDTH + outX + x] =
          static_cast<uint8_t>(((gray + 8u) / 17u) << 4) | static_cast<uint8_t>((alpha + 8u) / 17u);
    }
  }
  return true;
}

bool quantizeOpaqueToGc16(const uint8_t* gray, uint8_t* packed, const TaskControl& control) {
  ...
  auto distribute = [](int32_t error, int32_t weight) -> int16_t {
    ...
    return static_cast<int16_t>(weighted >= 0 ? (weighted + 8) / 16 : (weighted - 8) / 16);
  };
  ...
  for (int y = 0; y < TARGET_HEIGHT; ++y) {
    ...
    for (int scan = 0; scan < TARGET_WIDTH; ++scan) {
      ...
      int32_t adjusted =
          static_cast<int32_t>(gray[static_cast<size_t>(y) * TARGET_WIDTH + x]) * 16 + currentError[errorIndex];
...
bool computeSourceIdentity(const std::string& path, uint32_t& fileSize, uint32_t& fingerprint, uint32_t& pathHash) {
  FsFile file;
...
std::string makeCachePath(uint32_t pathHash, uint32_t fingerprint) {
  char name[96];
  snprintf(name, sizeof(name), "%s/%08lx_%08lx.sgc", CACHE_DIR, static_cast<unsigned long>(pathHash),
           static_cast<unsigned long>(fingerprint));
  return name;
}

bool validateCacheHeader(const SleepCacheHeader& header, size_t fileSize = 0) {
  if (std::memcmp(header.magic, "SLPC", 4) != 0 || header.version != CACHE_VERSION || header.width != TARGET_WIDTH ||
      header.height != TARGET_HEIGHT) {
    return false;
  }
  const uint32_t expected = header.type == CACHE_OPAQUE    ? static_cast<uint32_t>(OPAQUE_PAYLOAD_SIZE)
                            : header.type == CACHE_OVERLAY ? static_cast<uint32_t>(OVERLAY_PAYLOAD_SIZE)
                                                           : 0;
...
bool validateCacheFile(const std::string& path, SleepCacheHeader& header) {
  ...
  if (!readExact(file, &header, sizeof(header)) || !validateCacheHeader(header, size)) {
    file.close();
    return false;
  }
...
bool writeCache(const std::string& finalPath, CacheType type, const uint8_t* payload, size_t payloadSize,
                uint32_t sourceSize, uint32_t fingerprint, uint32_t pathHash) {
  ...
  const bool ok = writeExact(file, &header, sizeof(header)) && writeExact(file, payload, payloadSize);
  file.flush();
  file.close();
  if (!ok) {
    Storage.remove(tempPath.c_str());
...
int jpegDraw(JPEGDRAW* draw) {
  ...
  if (draw->x < 0 || draw->y < 0 || draw->x + validWidth > context->width ||
  ...
  for (int row = 0; row < draw->iHeight; ++row) {
    std::memcpy(context->gray + static_cast<size_t>(draw->y + row) * context->width + draw->x,
                pixels + static_cast<size_t>(row) * stride, validWidth);
  }
  return 1;
}
...
int chooseJpegScaleDenominator(int width, int height) {
  ...
  for (int denominator : candidates) {
    ...
    if (crop.width >= TARGET_WIDTH && crop.height >= TARGET_HEIGHT && pixels <= MAX_SOURCE_PIXELS) {
      return denominator;
    }
  }
  for (int denominator : candidates) {
    const size_t pixels =
        static_cast<size_t>((width + denominator - 1) / denominator) * ((height + denominator - 1) / denominator);
    if (pixels <= MAX_SOURCE_PIXELS) return denominator;
  }
  return 0;
}

bool decodeJpegToGray(const std::string& path, uint8_t*& grayOut, int& widthOut, int& heightOut,
                      const TaskControl& control) {
  JPEGDEC* jpeg = new (std::nothrow) JPEGDEC();
  if (!jpeg) return false;
  int rc = jpeg->open(path.c_str(), jpegOpen, jpegClose, jpegRead, jpegSeek, jpegDraw);
  if (rc != 1) {
    delete jpeg;
    return false;
  }
  ...
  if (originalWidth <= 0 || originalHeight <= 0 || originalWidth > MAX_SOURCE_DIMENSION ||
      originalHeight > MAX_SOURCE_DIMENSION) {
    jpeg->close();
    delete jpeg;
    return false;
  }
  ...
  auto* gray = static_cast<uint8_t*>(heap_caps_malloc(bytes, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT));
  if (!gray) {
    jpeg->close();
    delete jpeg;
    return false;
  ...
  LOG_INF("SLPCACHE", "JPEG decode: %dx%d at 1/%d -> %dx%d", originalWidth, originalHeight, denominator, decodedWidth,
          decodedHeight);

  rc = jpeg->decode(0, 0, option);
  jpeg->close();
  delete jpeg;
  if (rc != 1 || context.failed || shouldCancel(control) || context.maxX <= 0 || context.maxY <= 0) {
    heap_caps_free(gray);
    return false;
  }
  ...
  if (context.maxX != decodedWidth || context.maxY != decodedHeight) {
    const size_t compactBytes = static_cast<size_t>(context.maxX) * context.maxY;
    auto* compact = static_cast<uint8_t*>(heap_caps_malloc(compactBytes, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT));
    if (!compact) {
      heap_caps_free(gray);
      return false;
    }
    for (int y = 0; y < context.maxY; ++y) {
      std::memcpy(compact + static_cast<size_t>(y) * context.maxX, gray + static_cast<size_t>(y) * decodedWidth,
                  context.maxX);
    }
    heap_caps_free(gray);
    gray = compact;
...
bool inspectPng(const std::string& path, PngInfo& info) {
  ...
  uint8_t signature[8];
  if (!readExact(file, signature, sizeof(signature)) || std::memcmp(signature, "\x89PNG\r\n\x1a\n", 8) != 0) {
    file.close();
    return false;
  }
...
void convertPngLine(const PNGDRAW* draw, uint8_t* gray, uint8_t* alpha, int width) {
...
int pngDraw(PNGDRAW* draw) {
  ...
  if (!context || !context->gray || !context->alpha || draw->y < 0 || draw->y >= context->height ||
      shouldCancel(context->control)) {
    return 0;
  }
  if (!pauseForForeground(context->control)) return 0;
  convertPngLine(draw, context->gray + static_cast<size_t>(draw->y) * context->width,
                 context->alpha + static_cast<size_t>(draw->y) * context->width, context->width);
  return 1;
}

bool decodePng(const std::string& path, const PngInfo& info, uint8_t*& grayOut, uint8_t*& alphaOut,
               const TaskControl& control) {
  if (info.width <= 0 || info.height <= 0 || info.bitDepth != 8 || info.width > MAX_SOURCE_DIMENSION ||
      info.height > MAX_SOURCE_DIMENSION || static_cast<size_t>(info.width) * info.height > MAX_SOURCE_PIXELS) {
    LOG_ERR("SLPCACHE", "Unsupported PNG geometry/depth: %dx%d, %u-bit", info.width, info.height,
            static_cast<unsigned>(info.bitDepth));
    return false;
  }

  const size_t pixels = static_cast<size_t>(info.width) * info.height;
  auto* gray = static_cast<uint8_t*>(heap_caps_malloc(pixels, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT));
  auto* alpha = static_cast<uint8_t*>(heap_caps_malloc(pixels, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT));
  if (!gray || !alpha) {
  ...
  const int rcOpen = png->open(path.c_str(), pngOpen, pngClose, pngRead, pngSeek, pngDraw);
  ...
  const int bytesPerPixel = png->getPixelType() == PNG_PIXEL_TRUECOLOR_ALPHA ? 4
                            : png->getPixelType() == PNG_PIXEL_TRUECOLOR     ? 3
                            : png->getPixelType() == PNG_PIXEL_GRAY_ALPHA    ? 2
                                                                             : 1;
  const int requiredInternal = ((info.width * bytesPerPixel + 1) * 2) + 32;
#ifdef PNG_MAX_BUFFERED_PIXELS
  if (requiredInternal > PNG_MAX_BUFFERED_PIXELS) {
    LOG_ERR("SLPCACHE", "PNG row needs %d bytes, PNG_MAX_BUFFERED_PIXELS=%d", requiredInternal,
...
bool decodeBmpToTarget(const std::string& path, uint8_t* targetGray, const TaskControl& control) {
  FsFile file;
  ...
  const CropRect crop = calculateCenterCrop(sourceWidth, sourceHeight);
  auto* raw = static_cast<uint8_t*>(heap_caps_malloc(bitmap.getRowBytes(), MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT));
  auto* luminance = static_cast<uint8_t*>(heap_caps_malloc(sourceWidth, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT));
  if (!raw || !luminance) {
  ...
  for (int fileRow = 0; fileRow < sourceHeight; ++fileRow) {
    ...
    const int sourceY = bitmap.isTopDown() ? fileRow : sourceHeight - 1 - fileRow;
    if (sourceY < crop.y || sourceY >= crop.y + crop.height) continue;

    const int relativeY = sourceY - crop.y;
    const int destinationStart =
        static_cast<int>((static_cast<int64_t>(relativeY) * TARGET_HEIGHT + crop.height - 1) / crop.height);
    const int destinationEnd =
        static_cast<int>((static_cast<int64_t>(relativeY + 1) * TARGET_HEIGHT + crop.height - 1) / crop.height);
    for (int destinationY = destinationStart; destinationY < destinationEnd && destinationY < TARGET_HEIGHT;
         ++destinationY) {
      uint8_t* destinationRow = targetGray + static_cast<size_t>(destinationY) * TARGET_WIDTH;
      for (int x = 0; x < TARGET_WIDTH; ++x) {
        const int sourceX = crop.x + static_cast<int>((static_cast<int64_t>(x) * crop.width) / TARGET_WIDTH);
...
uint8_t sampleReaderPage(const uint8_t* physicalFrame, uint8_t orientation, int targetX, int targetY,
  ...
  int fitWidth = 0;
  int fitHeight = 0;
  calculateFitRect(sourceWidth, sourceHeight, fitX, fitY, fitWidth, fitHeight);
  if (targetX < fitX || targetY < fitY || targetX >= fitX + fitWidth || targetY >= fitY + fitHeight) {
    return 15;
  }

  const int sourceX =
      std::min(sourceWidth - 1, static_cast<int>((static_cast<int64_t>(targetX - fitX) * sourceWidth) / fitWidth));
  const int sourceY =
      std::min(sourceHeight - 1, static_cast<int>((static_cast<int64_t>(targetY - fitY) * sourceHeight) / fitHeight));

  if (clearBottomPixels > 0 && sourceY >= sourceHeight - std::min(sourceHeight, clearBottomPixels)) {
    return 15;
  }
  ...
  if (physicalX < 0 || physicalX >= HalDisplay::DISPLAY_WIDTH || physicalY < 0 ||
      physicalY >= HalDisplay::DISPLAY_HEIGHT) {
    return 15;
  }
  const uint8_t epd = physicalFrame[static_cast<size_t>(physicalY) * HalDisplay::DISPLAY_WIDTH + physicalX];
...
void collectSleepImageCandidates(const std::string& directory, std::vector<std::string>& output, const int depth = 0) {
  if (depth > 4) return;
...
void SleepImageManager::begin() {
  ...
  const size_t recentWindow =
      std::min<size_t>(APP_STATE.recentSleepFill, candidates.size() > 0 ? candidates.size() - 1 : 0);
  selectedCandidate = static_cast<size_t>(random(candidates.size()));
  for (uint8_t attempt = 0; attempt < 20 && APP_STATE.isRecentSleep(static_cast<uint16_t>(selectedCandidate),
...
void SleepImageManager::noteUserActivity() { lastUserActivityMs = millis(); }

void SleepImageManager::loop(uint32_t idleMs, bool foregroundBusy) {
  const bool customMayBeNeeded =
      SETTINGS.sleepScreen == CrossPointSettings::CUSTOM || SETTINGS.sleepScreen == CrossPointSettings::COVER_CUSTOM;
  if (!customMayBeNeeded || candidates.empty() || preparedReady || preparationAttempted || taskRunning ||
      foregroundBusy || idleMs < START_AFTER_IDLE_MS) {
    return;
  }
  ...
  taskRunning = true;
  const BaseType_t created = xTaskCreatePinnedToCore(taskTrampoline, "SleepImagePrep", 12288, this, 1, &taskHandle, 0);
  if (created != pdPASS) {
    preparationAttempted = false;
    taskRunning = false;
    taskHandle = nullptr;
...
void SleepImageManager::taskTrampoline(void* parameter) { static_cast<SleepImageManager*>(parameter)->prepareTask(); }
...
void SleepImageManager::prepareTask() {
  ...
  if (success && !cancelRequested) {
    ...
    preparedReady = true;
    LOG_INF("SLPCACHE", "Prepared successfully in %lu ms: %s", millis() - started, cachePath.c_str());
  } else if (!cancelRequested) {
    LOG_ERR("SLPCACHE", "No custom sleep image could be prepared; retrying after next boot");
  }
...
bool SleepImageManager::prepareCandidate(size_t candidateIndex, std::string& cachePathOut) {
  ...
  if (validateCacheFile(cachePath, existing) && existing.sourceSize == sourceSize &&
      existing.sourceFingerprint == fingerprint && existing.sourcePathHash == pathHash) {
  ...
  if (FsHelpers::hasJpgExtension(sourcePath)) {
    decoded = decodeJpegToGray(sourcePath, sourceGray, sourceWidth, sourceHeight, control);
    if (decoded) {
      outputGray = static_cast<uint8_t*>(heap_caps_malloc(TARGET_PIXELS, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT));
      decoded = outputGray && resampleOpaque(sourceGray, sourceWidth, sourceHeight, outputGray, control);
    }
    ...
    overlay = decoded && info.hasTransparency &&
              hasVisibleTransparency(sourceAlpha, static_cast<size_t>(sourceWidth) * sourceHeight);
    if (decoded && overlay) {
      overlayPayload =
          static_cast<uint8_t*>(heap_caps_malloc(OVERLAY_PAYLOAD_SIZE, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT));
      decoded = overlayPayload &&
                resampleOverlay(sourceGray, sourceAlpha, sourceWidth, sourceHeight, overlayPayload, control);
    } else if (decoded) {
      outputGray = static_cast<uint8_t*>(heap_caps_malloc(TARGET_PIXELS, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT));
      decoded = outputGray && resampleOpaque(sourceGray, sourceWidth, sourceHeight, outputGray, control);
    }
  ...
  if (overlay) {
    written =
        writeCache(cachePath, CACHE_OVERLAY, overlayPayload, OVERLAY_PAYLOAD_SIZE, sourceSize, fingerprint, pathHash);
  } else {
    auto* packed = static_cast<uint8_t*>(heap_caps_malloc(OPAQUE_PAYLOAD_SIZE, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT));
    if (packed && quantizeOpaqueToGc16(outputGray, packed, control) && !cancelRequested) {
      written = writeCache(cachePath, CACHE_OPAQUE, packed, OPAQUE_PAYLOAD_SIZE, sourceSize, fingerprint, pathHash);
    }
...
void SleepImageManager::captureReaderFrame(const uint8_t* physicalFrameBuffer, uint8_t readerOrientation) {
  if (!physicalFrameBuffer) return;
  if (!readerFrame) {
    readerFrame = static_cast<uint8_t*>(heap_caps_malloc(HalDisplay::BUFFER_SIZE, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT));
  }
  if (!readerFrame) {
  ...
  LOG_DBG("SLPCACHE", "Captured current reader framebuffer (orientation=%u)",
...
bool SleepImageManager::displayPreparedOrPrevious(HalDisplay& halDisplay, bool readerContext, bool rotate180,
  ...
  if (!preferred.empty() && displayCache(halDisplay, preferred, readerContext, rotate180, transparentBackgroundMode)) {
  ...
  if (last.length() > 0) {
    ...
    if (displayCache(halDisplay, path, readerContext, rotate180, transparentBackgroundMode)) {
...
bool SleepImageManager::displayCache(HalDisplay& halDisplay, const std::string& cachePath, bool readerContext,
                                     bool rotate180, uint8_t transparentBackgroundMode) {
  FsFile file;
  if (!Storage.openFileForRead("SLPCACHE", cachePath, file)) return false;
  SleepCacheHeader header{};
  if (!readExact(file, &header, sizeof(header)) || !validateCacheHeader(header, file.size())) {
    file.close();
    return false;
  }

  if (header.type == CACHE_OPAQUE) {
    auto* logical = static_cast<uint8_t*>(heap_caps_malloc(OPAQUE_PAYLOAD_SIZE, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT));
    if (!logical || !readExact(file, logical, OPAQUE_PAYLOAD_SIZE) ||
        fnv1aUpdate(2166136261u, logical, OPAQUE_PAYLOAD_SIZE) != header.payloadFingerprint) {
      if (logical) heap_caps_free(logical);
      file.close();
      return false;
    }
    file.close();
    const bool result = halDisplay.showGc16LogicalBuffer(logical, OPAQUE_PAYLOAD_SIZE, true, rotate180);
    heap_caps_free(logical);
    return result;
  }

  auto* overlay = static_cast<uint8_t*>(heap_caps_malloc(OVERLAY_PAYLOAD_SIZE, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT));
  auto* logical = static_cast<uint8_t*>(heap_caps_malloc(OPAQUE_PAYLOAD_SIZE, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT));
  if (!overlay || !logical || !readExact(file, overlay, OVERLAY_PAYLOAD_SIZE) ||
      fnv1aUpdate(2166136261u, overlay, OVERLAY_PAYLOAD_SIZE) != header.payloadFingerprint) {
  ...
  const bool useReaderPage = transparentBackgroundMode == CrossPointSettings::TRANSPARENT_SLEEP_CURRENT_READING_PAGE &&
                             readerContext && readerFrameValid && readerFrame != nullptr;
  const int readerSnapshotClearBottom =
      useReaderPage ? static_cast<int>(UITheme::getInstance().getStatusBarHeight()) + 2 : 0;

  for (int y = 0; y < TARGET_HEIGHT; ++y) {
    for (int x = 0; x < TARGET_WIDTH; ++x) {
      const uint8_t background =
          useReaderPage ? sampleReaderPage(readerFrame, readerFrameOrientation, x, y, readerSnapshotClearBottom) : 15;
      ...
      const uint8_t result = static_cast<uint8_t>((foreground * alpha + background * (15 - alpha) + 7) / 15);
      setPackedNibble(logical, TARGET_WIDTH, x, y, result);
    }
    if ((y & 31) == 31) vTaskDelay(1);
  }

  LOG_INF("SLP", "Transparent PNG background: %s", useReaderPage ? "current reading page" : "white");
  const bool result = halDisplay.showGc16LogicalBuffer(logical, OPAQUE_PAYLOAD_SIZE, true, rotate180);
